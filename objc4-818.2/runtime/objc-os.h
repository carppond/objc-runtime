...
/*
 spinlock_t 原本是一个 uint32_t 类型的非公平的自选锁,由于其安全问题,目前底层实现已由互斥锁os_unfair_lock锁代替,
 所谓非公平是指: 获得锁的顺序和申请锁的顺序无关,也就是说,第一个申请锁的线程有可能会最后一个获得锁,或者刚获得锁的线程会再次立刻获得该锁,造成其他线程忙等(busy_wait)
