...
/*
 ...
 template<typename T> class StripedMap 从数据结构上来看,它作为一个 key 是 void *,value 是 T 的 hash 表来用的.在 818 的源码中,全局搜索StripedMap,发现 T 作为 SileTable 和 spinlock_t 类型使用.
 - SideTable类型: StripedMap<SideTable>. SideTable 的使用:SideTable *table = &SideTable()[obj] 它的作用根据 objc_object 的指针计算出哈希值,然后从 SideTables 这张全局哈希表中找到 obj 对应的SideTable.
 - spinlock_t类型: StripedMap<spinlock_t>
 ...
 根据下面源码实现 Lock 的部分,发现抽象类型 T 必须支持 lock/unlock/forceReset/lockdebu_lock_precedes_lock 函数接口.已知 struct SideTable 都有提供.
...
/*
 ...
 在818 源码中全局搜索DisguisedPtr,发现抽象类型 T,仅作为 objc_object 和 objc_object *使用.而抽象类型 T 是 objc_object *时,用于隐藏__weak 变量的地址.
 */
