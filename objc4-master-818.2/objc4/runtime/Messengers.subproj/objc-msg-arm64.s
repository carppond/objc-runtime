...
.macro GetTaggedClass
    ...
    // 这里是 self 指针 & 0111(7),其实是获取低 3 位的 tag index,
	and	x10, x0, #0x7		// x10 = small tag
    // self 右移 55 位,保留高位 9 位,如果是有扩展的,这里获取的是 tagged pointer 标志和扩展 tag index
	asr	x11, x0, #55		// x11 = large tag with 1s filling the top (because bit 63 is 1 on a tagged pointer)
    // 比较 tag index 是否等于 7,结果影响 cpsr 标志
	cmp	x10, #7		// tag == 7?
    // tag = 7,表示当前类tagged pointer有扩展位,
    // 如果 tag == 7,x12 = x11;否则 x12 = x10
    ...
    /// 获取 tagged pointer 的类数组
	// x16 = _objc_debug_taggedpointer_classes[x12]
    // 表示在内存中的第几页
    // 获取tagged pointer数组所在页的地址(_objc_debug_taggedpointer_classes@PAGE左移 12 位赋值给 x10)
	adrp	x10, _objc_debug_taggedpointer_classes@PAGE
    // 获取到的地址加上偏移获取到tagged pointer 的类数组
	add	x10, x10, _objc_debug_taggedpointer_classes@PAGEOFF // 内存中的偏移
    // 从tagged pointer 数组中取出当前类.
    // 将 x12 左移 3 位 + x10 = x16,当前 x16 就是获取的类
...
.endmacro
    ...
    /*
     空检测和标记指针检查
     x0 是消息接收者 receiver,或者上方传递过来的 self
     这里主要是判断 self 是否是空(nil)和 self 是否是 tagged pointer 类型.
     */
	cmp	p0, #0			// nil check and tagged pointer check
#if SUPPORT_TAGGED_POINTERS
    /// 如果 self 是 tagged pointer 类型就调用 LNilOrTagged
	b.le	LNilOrTagged		//  (MSB tagged pointer looks negative)
#else
    // 如果 x0 == nil,就执行 LReturnZero
	b.eq	LReturnZero
#endif
	ldr	p13, [x0]		// p13 = isa
...
LGetIsaDone:
    ...
    // 调用函数实现 imp 或者 __objc_msgSend_uncached
	CacheLookup NORMAL, _objc_msgSend, __objc_msgSend_uncached

#if SUPPORT_TAGGED_POINTERS
LNilOrTagged:
    /// 如果 x0 == ni,就执行 LReturnZero
	b.eq	LReturnZero		// nil check
    /// 通过 tagged pointer 获取其 class
	GetTaggedClass
    // 调用CacheLookup,通过LGetIsaDone缓存中查找方法
	b	LGetIsaDone
