...
//OBJC_EXPORT void
//_objc_setClassLoader(BOOL (* _Nonnull newClassLoader)(const char * _Nonnull)) OBJC2_UNAVAILABLE;
...
// Map of tags to obfuscated tags. 标签到混淆标签的映射。
...
{
    // 将 _objc_makeTaggedPointer 的结果值与混淆器参数 objc_debug_taggedpointer_obfuscator 异或
    // 获取新的结果 value
    uintptr_t value = (objc_debug_taggedpointer_obfuscator ^ ptr);
    // ARM64 平台下
#if OBJC_SPLIT_TAGGED_POINTERS
    // 如果结果不支持混淆,或者配置了不使用混淆参数,就直接返回_objc_makeTaggedPointer的结果
    ...
    // 把结果 value的 tag index 位置清零
    value &= ~(_OBJC_TAG_INDEX_MASK << _OBJC_TAG_INDEX_SHIFT);
    // 把混淆后的 tag index 存储到 value
...
_objc_decodeTaggedPointer_noPermute(const void * _Nullable ptr)
{
    ...
    // ARM64 平台下
#if OBJC_SPLIT_TAGGED_POINTERS
    // 如果结果不支持混淆,或者配置了不使用混淆参数,ptr
    if ((value & _OBJC_TAG_NO_OBFUSCATION_MASK) == _OBJC_TAG_NO_OBFUSCATION_MASK)
        return value;
#endif
    // 将value与混淆器参数 objc_debug_taggedpointer_obfuscator 异或,获取原始指针
...
#if OBJC_SPLIT_TAGGED_POINTERS // ARM64 平台下
    // 获取基础的 tag index
    uintptr_t basicTag = (value >> _OBJC_TAG_INDEX_SHIFT) & _OBJC_TAG_INDEX_MASK;
    // 原始数据的 tag index 位置清零
    value &= ~(_OBJC_TAG_INDEX_MASK << _OBJC_TAG_INDEX_SHIFT);
    // 把原始的 tag index 存储到结果
...
/// 根据给定的标签和有效的负载(对象内容)创建一个tagged pointer对象
static inline void * _Nonnull
_objc_makeTaggedPointer(objc_tag_index_t tag, uintptr_t value)
{
    ...
    /// 判断 tag index 是否<= 6,其实是判断当前对象是否有扩展标志,是不是属于<=6的类
    if (tag <= OBJC_TAG_Last60BitPayload) {
        // ASSERT(((value << _OBJC_TAG_PAYLOAD_RSHIFT) >> _OBJC_TAG_PAYLOAD_LSHIFT) == value);
        /*
         1. ((uintptr_t)tag << _OBJC_TAG_INDEX_SHIFT) ：将 tag 的值左移 0 位（_OBJC_TAG_INDEX_SHIFT的值 0），这里相当于保留了 tag 的原始值。
         2. ((value << _OBJC_TAG_PAYLOAD_RSHIFT) >> _OBJC_TAG_PAYLOAD_LSHIFT)：先将value 的值左移4 位，再将结果右移 1 位。相当于把 value 的前 4 位去掉，去掉左移 4 位添加的一个 0，获取最终的值。
         3. 把第一步和第二步的值进行逻辑或运算，相当于把第一步的值放在了第二步值的最后 3 位上。
         4. 第三步的结果值和_OBJC_TAG_MASK进行逻辑或运算。_OBJC_TAG_MASK的值相当于  (1UL<<63)，相当于 1 后面跟着 63 个 0。经过逻辑或运算之后。结果的第一位变成了 1。也就是高位变成了 1。
         5. 将最终的结果赋值给变量 result，并当做参数传入 _objc_encodeTaggedPointer函数进行编码，并返回结果。
         */
        uintptr_t result =
            (_OBJC_TAG_MASK | 
        ...
        /*
         ...
         2. ((value << _OBJC_TAG_EXT_PAYLOAD_RSHIFT) >> _OBJC_TAG_EXT_PAYLOAD_LSHIFT)：先将 value 左移 12 位，再将结果右移 9 位。去掉value 的 前 12 位，在把末尾添加的 12 个 0，去掉 9 个，保留 3 个 0
         3. 第一步和第二步的值进行逻辑或。相当于把第一步的 9 位填到了第二步的前 9 位。
         4. 把第三部的结果值和_OBJC_TAG_EXT_MASK进行逻辑或。_OBJC_TAG_EXT_MASK相当于 以 1 开头，以 111 结尾的 64 位数据，这一步相当于把第三部的结果第一位变成 1，后 3 位变成 1.
         5. 将最终的结果赋值给变量 result，并当做参数传入 _objc_encodeTaggedPointer函数进行编码，并返回结果。
         */
        uintptr_t result =
            (_OBJC_TAG_EXT_MASK |
...
/// 对象的 ptr 是否是 nil 或者 tagged pointer
static inline bool
_objc_isTaggedPointerOrNil(const void * _Nullable ptr)
{
    // this function is here so that clang can turn this intob
